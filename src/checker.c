...
RecommendationList checkSpell(char *path, HashTable dictionary, HashTable suggestions) { // si es null es porque no se abrio el archivo o no hay recomendaciones
  ...
  while(flag) {
    ...
    if (c != '\n' && c != '!' && c != '?' && c != '.' && c != ',' && c != ';' && c != ':' && c != ' ' && c != EOF) {
      ...
      //   }
      // }
      // recommendations = inSuggestions(suggestions, word, i);
      if(sugg && (recommendations = inSuggestions(suggestions, word, i)) != NULL) {
        // printf("no me deberia mostrar\n");
...
RecommendationList searchRecommendations(RecommendationList list, char *word, HashTable dictionary, int wordLen, int line, HashTable suggestions) {
  ...
  for(int step = 1; step < 4 && lenSuggestions < 5; step++) {
    // printf("step:%d\n", step);
    if(step == 1) {
      // printf("_________1111___________\n");
    ...
    //   {
    //     printf("|%s|%p|, ", recommendations[i], recommendations[i]);
    //   }
    //   printf("\n");
    }

    if(step == 2) {
      // printf("__________2222__________\n");
      for(unsigned int i = 0; i < step1Table->length && lenSuggestions < 5; i++) {
        if(step1Table->elems[i] != NULL) {
          ...
          while(aux != NULL) {
            ...
            // printf("|%s|%d|\n",((WordPointer)(aux->elem))->word, auxWordLen);
            // printf("__________22split22__________\n");
            ...
            // printf("__________22delete22__________\n");
            ...
            // printf("__________22switch22__________\n");
            ...
            // printf("__________22replace22__________\n");
            ...
            // printf("__________22insert22__________\n");
    ...
    if(step == 3) {
      // printf("_________33333___________\n");
  ...
  // }
  // printf("\n");
  // for (int i = 0; i < step1Table->length; i++)
  // {
  //   if(step1Table->elems[i] != NULL) {
  //     List aux = step1Table->elems[i];
  //     while (aux != NULL)
  //     {
  //       printf("tabla: %s\n", ((WordPointer)(aux->elem))->word);
  //       if(strcmp(((WordPointer)(aux->elem))->word, "cacb") == 0) printf("asdflasldfasdkfdkmsafkjsdaf\n");
  //       aux = aux->next;
  //     }
